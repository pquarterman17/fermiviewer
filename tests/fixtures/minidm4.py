...
def _scalar(label: str, code: int, value: float) -> bytes:
    if code == 5:
...
def _string(label: str, s: str) -> bytes:
    payload = np.array([ord(c) for c in s], dtype="<u2").tobytes()
...
def write_mini_dm4(
    ...
    if data_type == 10:
        payload, elem_type = flat.astype("<u2").tobytes(), 4
    elif data_type == 2:
        payload, elem_type = flat.astype("<f4").tobytes(), 6
    else:
        raise ValueError(f"unsupported data_type {data_type}")

    dim_tags = b"".join(_scalar(str(k), 5, d) for k, d in enumerate(dims))

    cal = cal or []
    cal_dims = b""
    for k, c in enumerate(cal):
        inner = (_scalar("Scale", 7, c["scale"])
                 + _scalar("Origin", 7, c["origin"])
                 + _string("Units", c["units"]))
        cal_dims += _group(str(k), inner, 3)

    image_data_children = (
        _scalar("DataType", 5, data_type)
    ...
    root = _be32(4) + _be64(len(image_list) + 10) + _be32(1) + b"\x01\x01" + _be64(1) + image_list
