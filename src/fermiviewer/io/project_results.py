...
routes adapt (the same rule as `project_manifest.py`). Untrusted input is
assumed: ids and member paths are path-checked before any ZIP access, and
member payloads load with ``allow_pickle=False``.
"""
