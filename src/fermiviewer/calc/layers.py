...
def _refuse_traced_mask(mask: np.ndarray | None, waviness: bool) -> None:
    """Waviness tracing walks each column looking for one edge crossing.

    Over an irregular region a column is not a column — it is whatever
    slice of it the mask kept, with gaps — so the tracer would either
    find the edge at the region boundary or find nothing, and the
    roughness it reported would describe the drawn outline rather than
    the interface. Refusing beats a number that looks like metrology.
    """
    if mask is not None and waviness:
        raise ValueError(
            "waviness tracing needs whole columns and an irregular region "
            "does not give them; use a rectangular region, or waviness=False"
        )
