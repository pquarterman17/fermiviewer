...
def _resolved_edges(specs: list[EelsEdgeSpec]) -> list[ElementEdge]:
    ...
    The check itself is `calc.eels_quant.resolve_edge_element`, shared with
    the registered op rather than mirrored here: a route that rejects what
    its own recipe replay accepts is worse than either rule alone, and that
    is exactly what a second copy drifts into. This function is only the
    ValueError -> 422 adaptation (ADR 0005 §1).
    """
    out: list[ElementEdge] = []
    for spec in specs:
        try:
            symbol, z = resolve_edge_element(spec.element, spec.z)
