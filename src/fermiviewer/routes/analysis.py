    ...
    try:
        with value_error_as_422():
            pattern = index_spots_roi(
                ...
                # the typed pixel size is the column scale; the row extent comes
                # from the pattern's own calibration, a generated FFT's
                # reciprocal axes included (calc/diffraction_index.pattern_spacing)
                spacing=pattern_spacing(
                    ds.data.shape, ds.pixel_spacing, ds.pixel_unit, req.pixel_size_mm, cam
                ),
            )
