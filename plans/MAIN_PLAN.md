...
| SHAPE_ANALYSIS_PLAN.md | Shape identity for segmented objects: particle shape descriptors (circularity/aspect/Feret/orientation), advisory shape classes + aggregate flag, orientation rose, elliptic-Fourier similarity, circle/ellipse fitting | Active (created 2026-08-14 from the shape-identification audit; Wave 1 in a three-agent parallel build) | Own normative conventions section (Crofton circularity, axial orientation, 2D-projection caveat) and a frozen wire contract + file-ownership matrix that the parallel build depends on |
