...
| ANALYSIS_PRESENTATION_PLAN.md | Presenting/quantifying the app's own results: peak shapes (Voigt), fit residuals + R², σ bands on charts, vector chart export, Savitzky–Golay, population histograms, fit reports, batch/scripting reach | Active (7 open items; booked 2026-08-12 from `docs/originpro-jmp-gap-audit.md`; items 1/2/5/6/7 + item 3's first chart shipped same day in two waves) | Coherent campaign with its own scope rule (image-derived results only; general graphing/statistics DECLINED per the audit) and its own sequencing |
