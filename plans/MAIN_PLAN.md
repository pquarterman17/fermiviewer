...
| ANALYSIS_PRESENTATION_PLAN.md | Presenting/quantifying the app's own results: peak shapes (Voigt), fit residuals + R², σ bands on charts, vector chart export, Savitzky–Golay, population histograms, fit reports, batch/scripting reach | Active (5 open: item 3's last checkbox + Tier 3 build-on-demand; items 1/2/4/5/6/7/8 shipped 2026-08-12 in three same-day waves) | Coherent campaign with its own scope rule (image-derived results only; general graphing/statistics DECLINED per the audit) and its own sequencing |
