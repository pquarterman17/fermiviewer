...
| LASSO_EDITING_PLAN.md | Lasso-measure adjustment UX: true Douglas–Peucker at lasso close (the pref finally means what it says), retroactive Simplify, vertex delete + alt-drag edge insert, round ring handles | Active (created 2026-08-16 from an owner screenshot report; 5 items, 3 waves, cheapest-safe model per item) | Touches the pinned-ratchet MeasureOverlay and gesture code; carries its own conventions (stored-points-are-the-measurement, screen-px ε, spike preservation) and a frozen `simplifyRing` contract |
