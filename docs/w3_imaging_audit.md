...
| `watershed` | **hybrid** | `segment.watershed` (tranche 2) | port chamfer DT + grid-NMS auto-markers (deterministic, cheap); map flooding to `skimage.segmentation.watershed` on −D. Boundary pixels may differ from MATLAB's descending-D adoption flood → test region count exact, sorted areas at ±1 % tolerance, not per-pixel labels. |
