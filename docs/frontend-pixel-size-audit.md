...
| `lib/profileCsv.ts` `profileToCsv` | `position_px` column | `dist / pixel_size` | per-segment mapping from `endpointsPx` when anisotropic (a scalar quotient cannot recover pixels along a slanted line); px column dropped when the geometry is unknown; `# pixel_spacing:` header line |
