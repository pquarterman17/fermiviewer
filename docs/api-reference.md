...
| `region` | `list[record(kind, mode, bounds, outline, holes)]` | [] | no |  |  | region geometry in canonical 0-based inclusive (row, col) form; parts apply in order, empty = whole image. Mutually exclusive with roi |
...
| &nbsp;&nbsp;`region[].holes` | `list[2 x float]` | [] | no |  |  | [[row, col], ...] — ONE inner ring subtracted from this part |
