...
| `POST /api/diffraction/index` | Diffraction workshop. *`spots` rides a RowSpec list param, 1-based (row, col). An ROI gates WHICH spots are indexed and plays no part in the arithmetic: everything, centre/measured_r included, stays in the FULL-image frame. Re-framing into the ROI's own smaller size used to rescale every d-spacing by W_full/W_roi. A degenerate ROI errors instead of silently indexing everything with a shrunken d-scale* | `diffraction_index` | table | shipped |
