...
const number = (value: unknown): string => typeof value === "number" && Number.isFinite(value)
  ? new Intl.NumberFormat("en-US", { maximumSignificantDigits: 7 }).format(value)
  : escapeHtml(value);
...
function outputHtml(output: ReportOutput): string {
  ...
  if (output.kind === "scalar") {
    ...
    const unit = typeof output.data?.unit === "string" ? ` ${escapeHtml(output.data.unit)}` : "";
    return `<div class="metric"><div>${value}<small>${sigma}${unit}</small></div><span>${escapeHtml(output.name.replaceAll("_", " "))}</span></div>`;
  }
  if (output.kind === "curve") {
    const svg = curveSvg(output);
    return `<section class="output"><h3>${escapeHtml(output.name)}</h3>${svg || `<p class="unavailable">Curve values are stored in the project and are not embedded in this manifest.</p>`}${caption}</section>`;
  }
  if (output.kind === "table") {
    const rows = rowsOf(output);
    const columns = Array.isArray(output.data?.columns) ? output.data.columns : [];
    if (rows.length === 0) return `<section class="output"><h3>${escapeHtml(output.name)}</h3><p class="unavailable">Table values are stored in the project and are not embedded in this manifest.</p>${caption}</section>`;
    const head = columns.length > 0 ? `<thead><tr>${columns.map((column) => `<th>${escapeHtml(column)}</th>`).join("")}</tr></thead>` : "";
...
function resultHtml(result: ReportResult, options: ReportDocumentOptions): string {
  const selected = new Set(options.outputNamesByResult[result.id] ?? []);
  ...
  return `<section class="result">
    <header><div><span class="eyebrow">${escapeHtml(result.analysis)}</span><h2>${escapeHtml(result.label || result.analysis)}</h2></div><time>${escapeHtml(new Date(result.created_at).toLocaleString())}</time></header>
    <p class="source">Source: ${escapeHtml(sources || "No source recorded")}</p>
    ${scalar.length > 0 ? `<div class="metrics">${scalar.map(outputHtml).join("")}</div>` : ""}
    ${other.map(outputHtml).join("")}
    <details><summary>Parameters and provenance</summary><pre>${escapeHtml(JSON.stringify(result.params ?? {}, null, 2))}</pre><p>Result ${escapeHtml(result.id)} · FermiViewer ${escapeHtml(result.app_version || "unknown")}</p></details>
  </section>`;
}

export function resultsReportHtml(report: ResultsReport, options: ReportDocumentOptions): string {
  const calibration = report.calibration.map((entry) => `<tr><td>${escapeHtml(options.sourceNames[entry.image_id] ?? entry.image_id)}</td><td>${entry.consistent ? "Consistent" : "Multiple recorded states"}</td><td>${entry.variants.map((variant) => variant.axes.filter((axis) => axis.calibrated).map((axis) => `${number(axis.scale)} ${escapeHtml(axis.units)}/${axis.index < 2 ? "px" : "channel"}`).join(" · ") || "Uncalibrated").join("; ")}</td></tr>`).join("");
  ...
  <footer class="report-foot">Report manifest v${report.version}. Large member-backed arrays may require the originating FermiViewer project.</footer></main></body></html>`;
}
