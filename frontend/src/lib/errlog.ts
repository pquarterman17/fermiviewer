// Client-side error log (checklist O): window errors + rejections ring
// buffer, bundled into the bug report alongside the server log.
...
/** Install global listeners once (called from App). */
...
export async function downloadBugReport(): Promise<void> {
  ...
  a.href = url;
  a.download = `fv_bugreport_${Date.now()}.json`;
  a.click();
